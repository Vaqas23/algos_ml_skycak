class Matrix:
    ...
    def matrix_multiply(self, other_arr):
        if self.num_col != other_arr.num_row:
            ...
            for i in range(self.num_row):
                row_matrix = []
                for j in range(other_arr.num_row):
            ...
            self.num_row = len(resultant_matrix)
            self.num_col = len(resultant_matrix[0]) if resultant_matrix else 0
            return self
    ...
    def inverse_matrix_via_rref(self):
        ...
        self.num_col = len(self.matrix[0]) if self.matrix else 0
        return self
